    ...
    describe('POST /pokemons', () => {
        ...
        it('should return 400 Bad Request when missed require field', (done) => {
            request(app).post('/pokemons')
                .expect(400)
                .end((err, res) => {
                    res.body.should.deep.equal({ error: "Insufficient parameter: name and type are required parameters" })
                    done()
                })
        })
    })
    ...
    describe('PUT /pokemon/:id', () => {
        ...
        it('should return 400 Bad Request when try to update not existed pokemon', (done) => {
            request(app).put('/pokemons/1')
                .expect(400)
                .end((err, res) => {
                    res.body.should.deep.equal({ error: "Insufficient body: secondaryType is required body" })
                    done()
                })
        })
    })
...
describe('Integration Test', () => {
    it('GET /pokemon should return list of pokemons', (done) => {
